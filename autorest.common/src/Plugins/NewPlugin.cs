...
public abstract class NewPlugin :  AutoRest.Core.IHost
{
    ...
    public void Message(Message message) => _connection.Notify("Message", _sessionId, message).GetAwaiter().GetResult();
    public void WriteFile(string filename, string content, object sourcemap) => _connection.Notify("WriteFile", _sessionId, filename, content, sourcemap).GetAwaiter().GetResult();
