...
public class Program : NewPlugin
{
	...
	private static T GetXmsCodeGenSetting<T>(CodeModel codeModel, string name)
	{
		try
		{
	...
	protected override async Task<bool> ProcessInternal()
	{
		...
		new Settings
		{
			...
			ClientName = Program.GetXmsCodeGenSetting<string>(codeModelT, "name") ?? await GetValue("override-client-name"),
			PayloadFlatteningThreshold = Program.GetXmsCodeGenSetting<int?>(codeModelT, "ft") ?? await GetValue<int?>("payload-flattening-threshold") ?? 0,
		...
		Settings.Instance.CustomSettings.Add("InternalConstructors", Program.GetXmsCodeGenSetting<bool?>(codeModelT, "internalConstructors") ?? await GetValue<bool?>("use-internal-constructors") ?? false);
		Settings.Instance.CustomSettings.Add("SyncMethods", Program.GetXmsCodeGenSetting<string>(codeModelT, "syncMethods") ?? await GetValue("sync-methods") ?? "essential");
		Settings.Instance.CustomSettings.Add("UseDateTimeOffset", Program.GetXmsCodeGenSetting<bool?>(codeModelT, "useDateTimeOffset") ?? await GetValue<bool?>("use-datetimeoffset") ?? false);
		...
		// process
		var plugin = new AutoRest.Python.PluginPy();
